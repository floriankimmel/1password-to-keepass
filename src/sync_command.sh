...
keepassPassword=$(op read $keepassSecret)
...
op item --vault $vault list --format json | jq -r '.[].title' | while IFS= read -r title; do
    if printf '%s\n' "$keepass" | keepassxc-cli search  $keepassDatabase "$title" 2>&1 | grep -q "No results for that search term"; then
        password=$(op item get --vault $vault "$title" --format json | jq -r '.fields[] | select(.id == "credential" or .id == "password") | .value')
        username=$(op item get --vault $vault "$title" --format json | jq -r '.fields[] | select(.id == "username") | .value')
